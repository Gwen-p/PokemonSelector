...
public class CSVGenerator {
    ...
    public static void main(String[] args) {
        // Datos originales en formato de texto
        String data = """
...
""";
        ...
        createSimplifiedCSV(data, "pokemon_simplified.csv");
    }
//todo cambiar de español a english
    public static void createSimplifiedCSV(String data, String outputFile) {
        try {
            List<String> lines = Arrays.asList(data.split("\n"));
            List<String> simplifiedLines = new ArrayList<>();

            // Header del nuevo CSV para la clase Breed actualizada
            simplifiedLines.add("id,bName,pType,attack,HP");

            for (int i = 1; i < lines.size(); i++) { // Empezar desde 1 para saltar el header
                ...
                if (parts.length >= 6) {
                    ...
                    String pType = parts[2].trim(); // Solo Type 1
                    String attack = parts[5].trim();
                    String hp = parts[4].trim();

                    // Limpiar nombres con caracteres especiales
                    bName = cleanName(bName);

                    simplifiedLines.add(String.join(",", id, bName, pType, attack, hp));
                }
            }

            // Guardar el archivo simplificado
            Files.write(Paths.get(outputFile), simplifiedLines);
            System.out.println("✅ Archivo CSV creado: " + outputFile);
            System.out.println("📊 Total de Breeds procesados: " + (simplifiedLines.size() - 1));
            System.out.println("🎯 Formato: id,bName,pType,attack,HP");

            // Mostrar algunos ejemplos
            System.out.println("\n📝 Ejemplos:");
            for (int i = 1; i <= Math.min(5, simplifiedLines.size() - 1); i++) {
                System.out.println("   " + simplifiedLines.get(i));
            }
    ...
    private static String cleanName(String name) {
        // Limpiar caracteres especiales que puedan causar problemas en CSV
